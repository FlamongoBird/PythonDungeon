...
# and then tweaked/fixed some stuff to make it more to my liking.
...
def build_dungeon(p):
    ...
    if p.dungeon_level >= 100:
        map.place_rand_object(d, '@')
        util.log("Adding Boss To Level 100")
        loc = map.place_rand_object(d, "E")
        enemy = monsters.Enslaver()
        enemy.y = loc[0]
        enemy.x = loc[1]
        ...
        for i in range(random.randint(1, 10)):
            enemy = monsters.gen_random(p.dungeon_level)
            loc = map.place_rand_object(d, enemy.id)
            enemy.y = loc[0]
            enemy.x = loc[1]
            enemies.append(enemy)

        util.log("Adding Loot")
        _max = 5+int(p.dungeon_level/2)
        for i in range(random.randint(1, _max)):
            map.place_rand_object(d, 't')
    

    util.log("Adding Shards")
    if random.randint(1, 3) == 1:
        loc = map.place_rand_object(d, '*')
