...
class Game():
	...
	def gen_dungeon(self):
		...
		if self.player.dungeon_level == 100 and not self.player.enslaver_slayed:
			self.screen.display_card(self.player, ["You cannot leave this level until the enslaver", "Has been defeated"])
			return
	...
	def play(self):
		...
		while True:
			...
			for enemy in self.enemies:
				if enemy.dead:
					if enemy.id == "E":
						self.player.enslaver_slayed = True
						self.game_complete()
	...
	def game_complete(self):
		"""Enslaver slayed win version"""
		self.screen.display_card(content=["You slayed the Enslaver, the kingdom is now free!",
			"The choice is yours, you can continue", 
			"deeper into the unknown depths of this",
			"dungeon or you can leave, and live out",
			"the rest of your life with the riches",
			"you have earned."
		])
		util.cnt()

		confirm = self.screen.get_input(self.player, "Exit the dungeon now? Or continue exploring", raw_prompts=[("Exit", 0), ("Stay", 1)])


		if confirm == 0:
			self.screen.display_card(["You leave the dungeon, the kingdom is now safe."])
			util.cnt()
			self.leaderboard.add_record(self.player, self.filename)
			...
			self.screen.display_card(["Onward then brave adventurer! Godspeed!"])
			util.cnt()
