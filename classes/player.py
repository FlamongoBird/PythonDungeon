...
class Player():
	def __init__(self):
		...
		# Keep Track of Enslaver stuff
		self.enslaver_slayed = False
	...
	def restore(self, dict):
		...
		# Keep track of Enslaver stuff
		self.enslaver_slayed = dict['enslaver_slayed']
	...
	def give_boosts(self):
		...
		if self.character == "knight":
			...
			self.max_life = 75
			self.has_magic = False
			self.weapon = weapon.Weapon("Long Bow", 10, 10, 4)
			self.weapons = [self.weapon]
			...
			self.magic = 50
			self.max_magic = 50
			self.weapon = weapon.Weapon("Ancient Staff", 10, 5, 5)
			self.weapons = [self.weapon]
