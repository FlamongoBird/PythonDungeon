...
class Leaderboard():
	...
	def check_saves(self, save_num):
		"""Check if the player already has a spot on the leaderboard"""
		for user in self.leader_data:
			if user['save_num'] == save_num:
				return user


	def add_record(self, player, save_num):
		"""Add a record to the leaderboard"""

		if len(self.leader_data) < 20 or player.gold > self.leader_data[-1]['gold']:

			
			user = self.check_saves(save_num)
			...
			if not user:
				...
				user["depth"] = player.dungeon_level
